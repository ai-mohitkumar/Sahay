...
export const CrossDomainMoatBanner: React.FC<CrossDomainMoatBannerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'pushback' | 'synthesis'>('synthesis');
  ...
  const fetchBannerData = () => {
    Promise.all([
      ...
      .then(([synData, pushData]) => {
        ...
        // Default to pushback if overplanning is triggered, else synthesis
        if (pushData && pushData.is_pushback_triggered) {
          setActiveTab('pushback');
        } else {
  ...
  const hasPushback = pushback && pushback.is_pushback_triggered;

  return (
    <div className="space-y-3 mb-6">
      {/* Action Applied Success Notice */}
      {actionNotice && (
      ...
      {/* AI Insight Header & Tab Selector (Unified Single Voice Entrypoint) */}
      {hasPushback && (
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-1.5 p-1 bg-slate-950/90 rounded-2xl border border-slate-800 text-xs font-bold shadow-inner">
            <button
              onClick={() => {
                setActiveTab('pushback');
                setShowWorkOpen(false);
              }}
            ...
            >
              ...
              <span className="px-1.5 py-0.2 rounded-full bg-purple-500/30 text-purple-200 text-[10px] font-mono">
                {synthesis.reasoning_chain.confidence_pct}%
              </span>
            </button>
          </div>

          <span className="text-[11px] text-slate-400 hidden sm:inline font-medium">
            {activeTab === 'pushback' ? '1 additional insight available' : 'Active calibrated analysis'}
          </span>
        </div>
      )}

      {/* 1. Honest Pushback Card (When Active) */}
      {hasPushback && activeTab === 'pushback' && pushback && (
        <div className="bg-gradient-to-r from-amber-950/80 via-slate-900 to-amber-950/80 border border-amber-500/50 rounded-3xl p-5 shadow-2xl text-white animate-in fade-in duration-200">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-start gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-300 font-bold border border-rose-500/30 text-[11px] font-mono">
                    <span>↑</span>
                    <span>+{pushback.overplanning_delta_pct.toFixed(0)}% Over 30d Peak</span>
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2 shrink-0 w-full md:w-auto justify-end">
              ...
              >
                {showWorkOpen ? 'Hide Work ✕' : '🔍 Show Work'}
              </button>
              <button
                disabled={actionLoading}
                onClick={() => setShowDiffModal(true)}
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-bold text-xs shadow-lg transition disabled:opacity-50 flex items-center gap-1.5"
              >
                <span>⚡</span>
                <span>Cap to {pushback.recommended_safe_hours}h Focus Zone (Preview Diff)</span>
              </button>
            </div>
          </div>
          ...
          {showWorkOpen && (
            <div className="mt-4 pt-4 border-t border-amber-500/20 bg-slate-950/70 p-4 rounded-2xl animate-in fade-in duration-200 space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-amber-400">
                <span>Deductive Reasoning & Historical Grounding</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 font-mono">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-400"></span>
                  <span>{pushback.reasoning_chain.confidence_pct}% Confidence</span>
                </span>
              </div>
      ...
      {/* 2. Headline Cross-Domain Synthesis Card (When Active) */}
      {(!hasPushback || activeTab === 'synthesis') && (
        <div className="relative overflow-hidden bg-gradient-to-r from-indigo-950/90 via-slate-900 to-purple-950/90 border border-purple-500/30 rounded-3xl p-5 sm:p-6 shadow-2xl text-white animate-in fade-in duration-200">
          {/* Ambient Glow */}
          ...
          <div className="relative z-10 flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-purple-500/15 border border-purple-500/30 text-xs font-semibold text-purple-300">
                <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
                <span className="font-mono">{synthesis.reasoning_chain.confidence_pct}% Confidence</span>
              </div>
            </div>
